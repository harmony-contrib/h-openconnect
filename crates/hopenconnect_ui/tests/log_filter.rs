...
use hopenconnect_core::DiagnosticEntry;
use log_filter::{matches_log_filter_normalized, normalize_log_query, LogLevelFilter};

fn log(level: &str, message: &str, timestamp: &str) -> DiagnosticEntry {
    DiagnosticEntry {
...
fn matches_log_filter(log: &DiagnosticEntry, filter: LogLevelFilter, query: &str) -> bool {
...
fn filters_logs_by_level() {
    ...
    let warn = log("warn", "gateway retry", "102");
    ...
    assert!(matches_log_filter(&warn, LogLevelFilter::Warning, ""));
...
fn combines_level_and_query_filters() {
    let entry = log("error", "dns upstream failed", "103");
...
fn normalized_query_can_be_reused_across_the_log_batch() {
    let info = log("info", "Provider refresh succeeded", "104");
    let warning = log("warning", "Provider refresh failed", "105");
