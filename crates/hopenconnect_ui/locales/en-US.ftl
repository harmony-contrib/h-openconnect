...
empty-connections = No connections yet. Tap + or Scan above to add one.
...
conn-qr-export = Export connection QR code
conn-qr-scan = Scan
conn-qr-save = Save QR image
conn-qr-sensitive-hint = This QR code includes saved passwords and tokens. Share it only with trusted people. Certificate files are not included; select them again on another device.
conn-qr-too-large = This connection is too large for a QR code.
conn-qr-export-invalid = This connection cannot be exported as a QR code.
conn-qr-invalid = The QR code is not a valid H-OpenConnect connection
conn-qr-added-prefix = Added from QR code:
conn-qr-scan-failed-prefix = Scan failed:
conn-qr-saved = QR image saved
conn-qr-save-failed-prefix = Could not save QR image:
