...
local-lan = 允许本地局域网
force-global = 强制全局路由
force-global-desc = 忽略服务器分流，全部 IPv4 流量走隧道
...
logs-empty-subtitle = 核心和会话事件会显示在这里。
logs-level-all = 全部
about = 关于
language = 语言
theme = 主题
...
empty-connections = 还没有连接配置，点右上角添加或扫码。
form-required = 连接名称与服务器地址不能为空
feedback-connected = 已建立连接
feedback-disconnected = 连接已断开
feedback-failed = 连接失败，请查看错误信息与诊断日志
feedback-deleted = 连接配置已删除
conn-qr-export = 导出配置二维码
conn-qr-scan = 扫码
conn-qr-save = 保存二维码图片
conn-qr-sensitive-hint = 二维码包含已保存的密码和令牌，请勿公开分享。证书文件只包含本机路径，换设备后需重新选择。
conn-qr-too-large = 配置过大，无法生成二维码。
conn-qr-export-invalid = 此配置无法导出为二维码。
conn-qr-invalid = 二维码不是有效的 H-OpenConnect 配置
conn-qr-added-prefix = 已扫码添加：
conn-qr-scan-failed-prefix = 扫码失败：
conn-qr-saved = 二维码图片已保存
conn-qr-save-failed-prefix = 保存二维码失败：
auth-password = 密码
auth-certificate = 证书
