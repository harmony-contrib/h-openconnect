//! Initial safe-area snapshot captured by the ArkTS `hopenconnect.safe-area` plugin.
...
/// XComponent mounts. It still needs a Rust declaration so ability attaches
/// its session-scoped ArkTS context.
pub struct HOpenSafeAreaBridgePlugin;

impl BridgePlugin for HOpenSafeAreaBridgePlugin {
    type Mode = AsyncBridge;

    const ID: &'static str = "hopenconnect.safe-area";
...
mod tests {
    ...
    fn safe_area_plugin_matches_the_arkts_factory() {
        assert_eq!(HOpenSafeAreaBridgePlugin::ID, "hopenconnect.safe-area");
        assert_eq!(
            HOpenSafeAreaBridgePlugin::REQUIRED_CONTEXTS,
            &[BridgeContextRequirement::WindowStage]
        );
    }
